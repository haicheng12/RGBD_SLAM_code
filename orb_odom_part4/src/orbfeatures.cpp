...
namespace ORB_SLAM2
{
	...
	float ORBextractor::IC_Angle(cv::Mat image, cv::Point2f pt, std::vector<int> u_max)
	{
		int m_01 = 0, m_10 = 0;
	...
	void ORBextractor::computeDescriptors(cv::Mat image, std::vector<cv::KeyPoint> keypoints, cv::Mat& descriptors, std::vector<cv::Point> pattern)
	{
	...
	void ORBextractor::computeOrbDescriptor(cv::KeyPoint kpt, cv::Mat img, cv::Point* pattern, uchar* desc)
	{
...
#undef GET_VALUE
	...
	void ORBextractor::computeOrientation(cv::Mat image, std::vector<cv::KeyPoint> keypoints, std::vector<int> umax)
	{
	...
	std::vector<cv::KeyPoint> ORBextractor::DistributeOctTree(std::vector<cv::KeyPoint> vToDistributeKeys, const int minX,
	...
	bool ORBextractor::runORBextractor(cv::Mat image, std::vector<cv::KeyPoint>& keypoints, cv::Mat& descriptors)
	{
		if (image.empty())
			return false;
		...
		assert(image_.type() == CV_8UC1);

#ifdef OPENCV_ALGORITHM
		cv::Ptr<cv::ORB> orb = cv::ORB::create(nfeatures, scaleFactor, nlevels);
		orb->detectAndCompute(image_, cv::Mat(), keypoints, descriptors);
#else
		...
		for (int level = 0; level < nlevels; ++level)
		{
			...
			cv::Mat desc = descriptors_.rowRange(offset, offset + nkeypointsLevel);
			computeDescriptors(workingMat, keypoints_, desc, pattern);
			...
			if (level != 0)
			{
				...
				for (std::vector<cv::KeyPoint>::iterator keypoint = keypoints_.begin(),
					keypointEnd = keypoints_.end(); keypoint != keypointEnd; ++keypoint)
					keypoint->pt *= scale;
			}
